...
"""Iniciem la música de fondo"""
pygame.mixer.init()
mixer.music.load('10min_background_music.wav')
mixer.music.play()
...
while not done:
    ...
    for event in pygame.event.get():
        ...
        if end is True:
            ...
            """Finalitzem tots els sorolls
            mixer.stop()
            time.sleep(1)"""

            """Ending theme"""
            pygame.mixer.init()
            mixer.music.load('Ending_sound.wav')
            mixer.music.play()

            # big circle of the winner
            """pygame.draw.circle(screen, PLAYER_COLOR[curr_player],
                               (ROOM + 0.5 * SEP + (1 + 0.5) * (SLOT + SEP), 0.5 * SEP + (1 + 0.5) * (SLOT + SEP)),
                               RAD*3)"""
            time.sleep(6)
            pygame.mixer.quit()
            done = True
